...
def _cached_or_build(key, ttl, builder):
    cached = _cached_get(key, ttl=ttl)
    if cached is not None:
        return cached
    value = builder()
    _cached_set(key, value)
    return value
...
def fetch_controllers():
    try:
        result = _cached_or_build('controllers', 10, get_controllers)
...
def fetch_atis():
    try:
        result = _cached_or_build('atis', 10, get_atis)
        return jsonify(result)
    except Exception as e:
        return jsonify({"error": str(e)}), 500
...
def load_admin_documents():
    try:
        docs = _cached_or_build('admin_documents', 30, lambda: supabase.from_('site_documents').select('*').execute().data or [])
...
def load_public_documents():
    ...
    try:
        ...
        filtered = _cached_or_build('public_documents', 300, build)
...
def _fetch_all_user_rows(all_time=False, days=30, batch_size=1000):
    try:
        rows = []
        offset = 0
        while True:
            resp = supabase.rpc('get_users').range(offset, offset + batch_size - 1).execute()
            batch = resp.data or []
            rows.extend(batch)
            if len(batch) < batch_size:
                break
        ...
        current_app.logger.warning(f"get_users RPC failed: {e}")

    from datetime import date, timedelta
    since = None if all_time else (date.today() - timedelta(days=days - 1)).isoformat()

    try:
        return _fetch_all_rows('discord_users', 'created_at', since=since, batch_size=batch_size)
    except Exception as e:
        current_app.logger.warning(f"Fallback discord_users query failed: {e}")
        return []
...
def load_admin_clearances_daily():
    ...
    try:
        ...
        counts = _count_rows_by_date(rows)
        today = date.today()
        out = [
            {'date': (today - timedelta(days=i)).isoformat(), 'count': counts.get((today - timedelta(days=i)).isoformat(), 0)}
...
def _build_analytics_payload(full_history=False):
    ...
    user_rows = _fetch_all_user_rows(all_time=True)
    ...
    return {
        'metrics': {
            ...
            'trends': {
                'total_clearances': _trend_pct(last30_clearances, previous30_clearances),
                'today_clearances': _trend_pct(today_clearances, previous_today),
                'last7_clearances': _trend_pct(last7_clearances, previous7_clearances),
                'last15_clearances': _trend_pct(last15_clearances, previous15_clearances),
                'last30_clearances': _trend_pct(last30_clearances, previous30_clearances),
                'total_users': _trend_pct(total_users, previous_users),
            },
        },
        'charts': {
            'clearances_per_day': clearance_series if full_history else clearance_series[-30:],
...
def load_admin_analytics_overview():
    try:
        payload = _cached_or_build('admin_analytics_overview', 30, lambda: _build_analytics_payload(full_history=False))
...
def load_admin_analytics_all():
    try:
        payload = _cached_or_build('admin_analytics_all', 30, lambda: _build_analytics_payload(full_history=True))
        return jsonify(payload)
    except Exception as e:
...
def load_admin_user_growth():
    ...
    try:
        rows = _fetch_all_user_rows(all_time=all_time, days=days)
        from datetime import date, timedelta
        counts = {}
        for r in rows:
...
def _clearances_daily_series(since=None):
    rows = _fetch_all_rows('clearance_generations', 'created_at', since=since)
    counts = _count_rows_by_date(rows)
    return [{'date': k, 'count': v} for k, v in sorted(counts.items())], counts


def analytics_clearances_per_day():
    try:
        def build():
            series, _ = _clearances_daily_series()
            return series
        out = _cached_or_build('analytics_clearances_per_day', 30, build)
        return jsonify(out)
    except Exception as e:
...
def analytics_clearances_last_n(days):
    try:
        def build():
            from datetime import date, timedelta
            since = (date.today() - timedelta(days=days - 1)).isoformat()
            _, counts = _clearances_daily_series(since=since)
            today = date.today()
            return [
                {'date': (today - timedelta(days=i)).isoformat(), 'count': counts.get((today - timedelta(days=i)).isoformat(), 0)}
                for i in range(days - 1, -1, -1)
            ]
        out = _cached_or_build(f'analytics_clearances_last_n:{days}', 30, build)
        return jsonify(out)
    except Exception as e:
