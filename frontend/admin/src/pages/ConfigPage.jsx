...
const ConfigPage = () => {
    ...
    const handleSave = () => {
        const payload = { ...settings, defaultSettingsEnabled: Boolean(defaultSettingsEnabled) };
        try {
    ...
    const handleReset = () => {
        ...
        saveUserSettings({ ...DEFAULT_SETTINGS, defaultSettingsEnabled: false });
    ...
    const renderTemplatePreview = () => {
        ...
        return parts.map((part, idx) => {
            if (part.match(/^\[[^\]]+\]$/)) {
                return (
                    <span key={idx} className="bg-primary/20 text-primary font-semibold px-1.5 py-0.5 rounded border border-primary/40">
                        {part}
                    </span>
                );
            }
    ...
    const handleDefaultSettingsToggle = (enabled) => {
        setDefaultSettingsEnabled(Boolean(enabled));
        ...
        <main className="flex-1 overflow-y-auto p-4 md:p-6 lg:p-8 space-y-8 pt-20 lg:pt-8">
            ...
            <div className="bg-surface-dark border border-border-dark rounded-lg p-6 shadow-sm">
                ...
                <div className="flex items-center gap-3">
                    ...
                    <button onClick={() => setShowPreview(!showPreview)} className="bg-zinc-800 hover:bg-zinc-700 text-white font-bold uppercase tracking-widest py-2 px-6 rounded transition-all text-sm flex items-center gap-2">
                        <span className="material-symbols-outlined text-lg">visibility</span>
                        Preview
                    </button>
                    <button onClick={() => setShowExport(!showExport)} className="bg-zinc-800 hover:bg-zinc-700 text-white font-bold uppercase tracking-widest py-2 px-6 rounded transition-all text-sm flex items-center gap-2">
                ...
                {showPreview && (
                    <div className="mt-4 p-4 bg-black/30 border border-zinc-800 rounded">
                        ...
                        <p className="font-mono text-sm text-zinc-300 leading-relaxed whitespace-pre-wrap break-words">{renderTemplatePreview()}</p>
                    </div>
                )}
                ...
                {showExport && (
                    <div className="mt-4 p-4 bg-black/30 border border-zinc-800 rounded">
                        ...
                        <div className="bg-black/50 border border-zinc-800 rounded p-3 font-mono text-sm text-zinc-300 mb-3 max-h-32 overflow-y-auto break-words">{exportTemplateWithBraces()}</div>
                        <button onClick={copyExportToClipboard} className="bg-primary hover:brightness-110 text-black font-bold text-xs py-2 px-4 rounded transition-all flex items-center gap-2">
            ...
            <div className="bg-surface-dark border border-border-dark rounded-lg p-6 shadow-sm">
                ...
                <div className="flex items-center gap-3 mb-5">
                    ...
                    <button type="button" onClick={() => handleDefaultSettingsToggle(!defaultSettingsEnabled)} className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${defaultSettingsEnabled ? "bg-primary" : "bg-zinc-700"}`} aria-pressed={defaultSettingsEnabled} aria-label="Toggle default settings">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    <label className="space-y-1.5">
                        ...
                        <div className={!defaultSettingsEnabled ? "pointer-events-none opacity-50" : ""}>
                            <Combobox options={routingOptions} value={settings.defaultRouting} onChange={handleDefaultRoutingChange} placeholder="Select routing" />
                        </div>
                    </label>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                    <label className="flex items-center gap-3 bg-black/30 border border-zinc-800 rounded px-4 py-3">
                        <input type="checkbox" checked={settings.uppercaseCallsign} onChange={(e) => updateSetting("uppercaseCallsign", e.target.checked)} disabled={!defaultSettingsEnabled} className="h-4 w-4 accent-primary" />
                ...
                <button onClick={handleReset} className="text-zinc-500 hover:text-white px-4 py-2 text-sm uppercase tracking-wider font-bold transition-colors">Reset All to Default</button>
            </div>
        </main>
    );
};
