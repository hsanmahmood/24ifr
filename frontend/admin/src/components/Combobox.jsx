            ...
            <div className="relative">
                ...
                >
                    <div className="flex items-center gap-2 truncate">
                        <span className={`truncate ${selectedOption ? 'text-white' : 'text-zinc-500'}`}>{selectedOption ? selectedOption.label : placeholder}</span>
                        {selectedOption?.subtext && (
                            <span className="text-xs font-bold text-emerald-500 bg-emerald-500/10 px-2 py-0.5 rounded border border-emerald-500/30">
                                {selectedOption.subtext}
                            </span>
                        )}
                    </div>
                ...
                {isOpen && (
                    <div className="absolute z-50 left-0 right-0 top-full mt-1 bg-surface-dark border border-zinc-800 rounded-lg shadow-xl overflow-hidden">
                        ...
                        <div className="max-h-56 overflow-y-auto custom-scrollbar p-1">
                            {filteredOptions.length > 0 ? (
                                filteredOptions.map((option) => {
                                    ...
                                    const baseTextClass = isSelected ? 'text-primary' : (option.offline ? 'text-zinc-600 opacity-60' : 'text-zinc-300');
                                    const rowHoverClass = option.offline ? '' : 'hover:bg-zinc-800';
                                    return (
                                        <div
                                            key={option.value}
                                            className={`px-3 py-2 text-xs rounded cursor-pointer transition-colors flex justify-between items-center group ${isSelected ? 'bg-primary/20' : ''} ${baseTextClass} ${rowHoverClass}`}
                                            onClick={() => {
                                        ...
                                        >
                                            ...
                                            {option.subtext && <span className={`text-[10px] ${isSelected ? 'text-primary/70' : (option.offline ? 'text-zinc-500' : 'text-zinc-600 group-hover:text-zinc-500')}`}>{option.subtext}</span>}
                                        </div>
                                    );
                                })
                            ) : (
