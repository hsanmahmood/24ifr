...
const Sidebar = () => {
    ...
    const navItems = [
        { to: '/', icon: 'home', label: 'Main', end: true },
                ...
                <div className="lg:hidden fixed inset-0 z-40 bg-background-dark/95 backdrop-blur-sm animate-fadeIn flex flex-col pt-20 px-6">
                    <nav className="space-y-4">
                        {navItems.map((item) => (
                            <NavLink
                                key={item.to}
                                className={({ isActive }) => [
                                    'group relative flex items-center rounded-[6px] py-2.5 text-sm font-medium transition-[150ms] ease-out gap-3 px-3',
                                    isActive
                    ...
                    <div className="mt-auto pb-10">
                        {authLoading ? (
                            ...
                            <button
                                onClick={import('../services/api').then(m => m.loginWithDiscord())}
                                className="flex w-full items-center justify-center gap-2 rounded-[6px] bg-[#f5c518] py-4 text-sm font-semibold text-black transition-[150ms] ease-out hover:brightness-95"
                            >
